...
def web(
    ...
    share: bool = typer.Option(
        False, "--share", help="Create a public Gradio share link."
    ),
):
    """Launch the TradingAgents Web UI (Gradio-based)."""
    try:
        from web.app import main as web_main
    except ImportError:
        console.print(
            "[red]Error:[/red] Gradio is not installed.\n"
            "Install web extras with:\n"
            "  [bold]uv sync --extra web[/bold]\n"
            "or:\n"
            "  [bold]uv pip install gradio>=5.0[/bold]"
        )
        raise typer.Exit(1)
    console.print(
        f"[green]Launching TradingAgents Web UI on http://{host}:{port}[/green]"
    )
    web_main(host=host, port=port, share=share)
